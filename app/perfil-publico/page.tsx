...
async function PerfilPublicoContent({ id }: { id: string }) {
  ...
  const { data: quizzes } = await supabase
    .from("quizzes").select("*, questions(id)")
...
async function PerfilPublicoWrapper({ searchParamsPromise }: { searchParamsPromise: Promise<{ id: string }> }) {
  const { id } = await searchParamsPromise;
  if (!id) notFound();
  return <PerfilPublicoContent id={id} />;
}
  ...
  return (
    <PageLayout>
      <Suspense fallback={
        ...
        <PerfilPublicoWrapper searchParamsPromise={searchParams} />
      </Suspense>
    </PageLayout>
  );
}